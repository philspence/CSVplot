...
class MainFrame(wx.Frame):
    ...
    def setUI(self):
        ...
        self.bg_sub_chk = wx.CheckBox(self.panel, label="Background Subtract")
        self.zero_correct_chk = wx.CheckBox(self.panel, label="Zero Correct (at longest wavelength)")
        ...
        self.hbox_Linear = wx.BoxSizer(wx.HORIZONTAL)
        slope_txt = wx.StaticText(self.panel, label="Slope:")
        self.slope_box = wx.TextCtrl(self.panel, style=wx.TE_READONLY, value="", size=(50, 25))
        intercept_txt = wx.StaticText(self.panel, label="Intercept:")
        self.intercept_box = wx.TextCtrl(self.panel, style=wx.TE_READONLY, value="", size=(50, 25))
        slope_intercept_txt = wx.StaticText(self.panel, label="Slope/Intercept:")
        self.slope_intercept_box = wx.TextCtrl(self.panel, style=wx.TE_READONLY, value="", size=(50, 25))
        self.hbox_Linear.AddMany([(slope_txt, 0, wx.ALL, 5),
        ...
        c_txt = wx.StaticText(self.panel, label="Concentration (uM):")
        self.c = wx.TextCtrl(self.panel, size=(50, 25))
        NLBindingConst_txt = wx.StaticText(self.panel, label="Kb (uM):")
        ...
        self.hbox_Nonlinear.AddMany([(c_txt, 0, wx.ALL, 5),
                                     (self.c, 0, wx.ALL, 5),
                                     (NLBindingConst_txt, 0, wx.ALL, 5),
    ...
    def load_csv_press(self, event):
        input_path, csv_filename, csv_dir = load_csv(self)
    ...
    def plot_btn_press(self, event):
        ...
        data = opentext(self.input_path_txtbx.GetLabel(), int(self.set_headers_tb.GetValue()))
        ...
        if zero_corr == True:
            ...
            self.y = zero_correct(zero_pos, self.y)
        else:
            pass
    ...
    def SecPlotChoice(self, event):
        ...
        if self.sec_plot_type.GetCurrentSelection() == 1:   #linear
            self.vbox_SecParams.Hide(self.hbox_Nonlinear)
            self.vbox_SecParams.Hide(self.hbox_CDMelt)
            self.vbox_SecParams.Show(self.hbox_Linear)
            resize_sizer(self, self.vbox_SecParams)
    ...
    def save_fig_1(self, event):
        path = save_csv(self)
        extent = full_extent(self.axes1).transformed(self.figure.dpi_scale_trans.inverted())
        self.figure.savefig(path, bbox_inches=extent, dpi=600)

    def save_fig_2(self, event):
        path = save_csv(self)
        extent = full_extent(self.axes2).transformed(self.figure.dpi_scale_trans.inverted())
        self.figure.savefig(path, bbox_inches=extent, dpi=600)
